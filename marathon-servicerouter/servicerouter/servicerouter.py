...
def reloadConfig():
    ...
    if os.getenv("HAPROXY_RELOAD_CMD"):
        logger.debug("HAPROXY_RELOAD_CMD is set, using it as command for reload")
        reloadCommand = os.getenv("HAPROXY_RELOAD_CMD").split(" ")
    elif os.path.isfile('/etc/init/haproxy.conf'):
